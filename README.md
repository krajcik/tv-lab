...
- По умолчанию — золотистый ретривер, курильский бобтейл и рэгдолл.
- Своя картинка через системный выбор файлов заменяет стандартных животных.
- 8 / 16 / 32 тысячи частиц, спокойный или обычный темп.
...
Стандартные животные — [сгенерированные иллюстрации](docs/assets.md).

![Ретривер из частиц — кадр Android TV эмулятора](docs/screenshots/retriever.png)

### Установка и запуск

Требуются Android 9.0+ (API 28) и OpenGL ES 2.0. Целевой телевизор —
Philips 65PUS8729/60. Работоспособность на нём пока не проверена.
...
системные настройки. Если выбора нет, автоматическое включение ещё потребует
отдельной проверки конкретной прошивки. Предпросмотр доступен независимо от этого.
...
Фактически выполненные проверки перечислены в [docs/validation.md](docs/validation.md).
Устройство движка и ограничения первого выпуска — в [docs/architecture.md](docs/architecture.md).
