...
final class ParticleRenderer implements GLSurfaceView.Renderer {
    ...
    private int modeUniform,pointMode,fieldMode,wallUniform;
    ...
    @Override public void onSurfaceCreated(GL10 unused,EGLConfig config) {
        ...
        modeUniform=GLES30.glGetUniformLocation(simulation,"uMode");
        pointMode=GLES30.glGetUniformLocation(points,"uMode");
        ...
        fieldMode=GLES30.glGetUniformLocation(fieldProgram,"uMode");
        wallUniform=GLES30.glGetUniformLocation(fieldProgram,"uWalls[0]");
    ...
    void renderFrame(float seconds){
        ...
        if(targetScene!=engine.sceneIndex()){
            Log.i("ParticlesPerf","scene="+engine.sceneIndex()+" mode="+engine.mode()+" duration="+engine.timeline.duration);
        ...
        GLES30.glUseProgram(simulation);uploadMode(modeUniform);GLES30.glUniform1i(fieldUniform,0);GLES30.glUniform1i(meanUniform,1);
        ...
        GLES30.glUseProgram(points);uploadMode(pointMode);GLES30.glUniform2f(pointPixels,scale,ratio);
    ...
    private void uploadMode(int location){GLES30.glUniform4f(location,engine.mode(),engine.phase,0,engine.modeBlend());}
    private void buildFieldOnGpu(){
        GLES30.glBindVertexArray(emptyVao);GLES30.glEnable(GLES30.GL_RASTERIZER_DISCARD);
        GLES30.glUseProgram(fieldProgram);uploadMode(fieldMode);
        GLES30.glUniform4fv(wallUniform,4,engine.field.walls,0);
    ...
    private int program(int vertex,int fragment,String[] varyings){
        int vs=shader(GLES30.GL_VERTEX_SHADER,read(vertex).replace("// MODE_FUNCTIONS",vertex==R.raw.simulation_vert?read(R.raw.modes):"")),fs=shader(GLES30.GL_FRAGMENT_SHADER,read(fragment));
