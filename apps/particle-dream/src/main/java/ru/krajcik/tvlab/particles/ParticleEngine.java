...
public final class ParticleEngine {
    ...
    public void step(float seconds) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            float settled=FlowField.smooth(peak-.6f,peak+.4f,local)*(1-FlowField.smooth(release,release+.8f,local));
            float desired = properties[at+3] > .5f ? 0 : Math.min(.99f,likeness*(words?1.02f:1.0626f*(.9f+.1f*(float)Math.sin(time*2.7f+seed*9))));
            if(properties[at+3]<.5f)desired+=(Math.min(.99f,likeness*1.02f)-desired)*settled;
            ...
            float clarity=settled*FlowField.smooth(.65f,.98f,capture);
            float offset=1+((words?.15f:.35f)-1)*clarity;
            float px = targets[target]+properties[at+4]*offset, py = targets[target+1]+properties[at+5]*offset;
            float noise=(words?.002f:.011f)+((words?.00015f:.0006f)-(words?.002f:.011f))*clarity;
            ...
            float freedom = (.28f+.72f*(1-capture)*(1-capture))*(words?.36f:1)*(1-.92f*clarity);
            float pull = ((words?90:30)+((words?160:75)-(words?90:30))*clarity)*capture*capture*capture;
            float damping = ((words?16.1f:6.1f)+((words?22:14)-(words?16.1f:6.1f))*clarity)*capture, layer = .8f+(int)(seed*4)*.15f, response=1.8f+(seed%.25f)*4;
