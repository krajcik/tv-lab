...
final class GpuParityTest {
    static String run(Instrumentation instrumentation, Activity activity) throws Exception {
        ...
        float[] maximum={0};
        float[] perComponent=new float[6];
        ...
        instrumentation.runOnMainSync(()->{
            ...
            ParticleEngine gpu=new ParticleEngine(256,new ScenePlaylist(new float[][]{{0,0,1,1}},new boolean[]{true}),18.75f,0);
            ParticleEngine cpu=new ParticleEngine(256,new ScenePlaylist(new float[][]{{0,0,1,1}},new boolean[]{true}),18.75f,0);
                ...
                public void onDrawFrame(GL10 gl){
                    ...
                    try{
                        for(int frame=0;frame<900;frame++){
                            renderer.renderFrame(.045f);cpu.step(.045f);
                            ...
                            for(int i=0;i<actual.length;i++){
                                ...
                                perComponent[i%6]=Math.max(perComponent[i%6],Math.abs(actual[i]-cpu.state[i]));
                            }
                            if(maximum[0]>.0002f)throw new AssertionError("Local GPU/CPU divergence="+maximum[0]+"; components="+java.util.Arrays.toString(perComponent));
                            renderer.synchronizeTestState(cpu.state);
                        }
        ...
        return "GPU/CPU max error="+maximum[0]+" (256 particles, 900 steps, synchronized 60-step blocks including 20-second text hold and release)";
    }
}
